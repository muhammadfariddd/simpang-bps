    ...
    <div class="page-header">
        <div class="page-block">
            <div class="row align-items-center">
                <div class="col">
                    <div class="page-header-title"><h5>Detail Proyek</h5></div>
                </div>
                <div class="col-auto">
                    <ul class="breadcrumb">
        ...
        <div class="col-xl-7">
            <div class="card" style="margin-bottom:16px;">
                <div class="card-header" style="display:flex;align-items:center;justify-content:space-between;">
                    ...
                    <span class="badge {{ $proyek->status === 'selesai' ? 'bg-success' : 'bg-primary' }}" style="font-size:12px;">
                ...
                <div class="card-body">
                    ...
                    <div style="margin:16px 0;">
                        <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
                            ...
                            <span style="font-weight:700;color:#4680ff;font-size:18px;">{{ $proyek->progress_persen }}%</span>
                        </div>
                        <div style="background:#e7eaee;border-radius:8px;height:14px;">
                            <div style="width:{{ $proyek->progress_persen }}%;background:#4680ff;height:14px;border-radius:8px;transition:width 0.5s;"></div>
                        </div>
                    </div>
                    @if($proyek->file_laporan)
                        <a href="{{ asset('storage/' . $proyek->file_laporan) }}" target="_blank" class="btn btn-outline-success btn-sm">
                    ...
                    @forelse($proyek->milestones as $ms)
                        <div style="border:1px solid #e7eaee;border-radius:8px;padding:12px;margin-bottom:10px;">
                            ...
                            <div style="background:#e7eaee;border-radius:4px;height:6px;margin-bottom:4px;">
                                <div style="width:{{ $ms->progress_persen }}%;background:#4680ff;height:6px;border-radius:4px;"></div>
                            </div>
                            ...
                            @if(Auth::user()->peran === 'mahasiswa')
                                <form method="POST" action="{{ route('milestone.update', $ms->id) }}" style="margin-top:8px;display:flex;gap:6px;align-items:center;">
                                    @csrf @method('PATCH')
                                    <select name="status" class="form-control" style="width:auto;font-size:12px;">
                                        <option value="belum"  {{ $ms->status === 'belum'  ? 'selected' : '' }}>Belum</option>
                                        <option value="proses" {{ $ms->status === 'proses' ? 'selected' : '' }}>Proses</option>
                                    ...
                                    <input type="number" name="progress_persen" value="{{ $ms->progress_persen }}"
                                        class="form-control" style="width:80px;font-size:12px;" min="0" max="100">
                    ...
                    <form method="POST" action="{{ route('proyek.milestone.store', $proyek->id) }}">
                        ...
                        <div class="row">
                            <div class="col mb-3">
                                ...
                                <input type="number" name="progress_persen" class="form-control" value="0" min="0" max="100">
                            </div>
                            <div class="col mb-3">
